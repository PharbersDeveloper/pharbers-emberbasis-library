	...
	model.eachRelationship((key, descriptor) => {
		...
		if (descriptor.kind === 'hasMany') {
			...
			model.get(key).toArray().forEach(result => {
				data.push({
					id: result.get('id'),
					type: result.store.serializerFor(result._internalModel.modelName).payloadKeyFromModelName(descriptor.type)
				});
			});
		} else {
			relationshipKey['data'] = {
				id: model.get(key).get('id'),
				type: model.store.serializerFor(model._internalModel.modelName).payloadKeyFromModelName(descriptor.type)
			};
		}
	});
}
